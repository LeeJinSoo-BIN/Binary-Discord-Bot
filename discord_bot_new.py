...
async def send_message(channel_id, msg, msg_type = "normal"):    
    channel = client.get_channel(channel_id)    
    if msg_type == "status" :        
        '''idx1 = msg.find("/")
        idx2 = msg.find(")")
        date = int(msg[idx1+2:idx2-1])
        msg_length = 50 + (50 + date*10) * msg.count(')')
        if msg_length > 2000 :
            cut_cnt = 1950 // (50+date*10)
            cut_cnt * (50+date*10)
        '''
        ...
        if len(msg) > 2000 :
            ...
            await send_message(channel_id, msg2, msg_type="status")
        else:
...
def crawl(url, cookies={}, headers={"User-Agent": "Mozilla/5.0"}, save_html=False, filepath="tmp.html",
          open_page=False):
...
async def print_memeber_status():
    ...
    if code_block:
        text += "```"
    await send_message(channel_id, text, msg_type = "status")
    print("done!!!")
...
async def alarm_every_Oclock():
    ...
    while (True):
        ...
        if err == 0 and datetime.now().hour == 23 and datetime.now().minute == 59 and datetime.now().second == 00:
            err = 1
            await print_memeber_status()
        else:
            err = 0
